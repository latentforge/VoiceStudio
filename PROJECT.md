...
|---|---|---|---|---|
| Breeze TTS 2 | Yes | Yes | Yes | `2cc89500`. Inherits CSM for the depth decoder, Qwen3 for the backbone; native `T5Gemma2TextEncoder` and `MimiModel`. wav2vec2 transcribed the generated speech; 1115 tensors load clean. Gaps in the folder README. The bundled audio tokenizer's key remap is a conversion mapping registered for `Qwen3TTSTokenizerMultiCodebookConfig` in `voicestudio/models/qwen3_tts/processing_qwen3_tts.py`, so the `audio_tokenizer` subfolder is read as it stands and the folder's `weight_conversion.py` is gone; the tokenizer that builds is bit for bit the one the old conversion produced over all 512 tensors, the same generated frames decode to identical waveforms across three seeds, and those three transcribe verbatim under `whisper-large-v3-turbo`. `AutoProcessor.from_pretrained` does not work on `BreezeBlue/breeze-tts-2`: its `tokenizer_config.json` declares `processor_class: Gemma3Processor`, which `transformers` follows into an image processor that the repository does not carry, so the README calls `BreezeTTSProcessor.from_pretrained` instead. Open, and older than this entry. |
...
| ECAPA-TDNN (`ecapa_tdnn`) | Yes | Yes | n/a (SpeechBrain is a toolkit rather than a model repository and was never vendored; its `eval` extra entry was removed in `6b407586`) | A speaker embedding model, so section 2.5's transcription standard does not apply and the numeric check against upstream's own classes is the evidence. Against SpeechBrain 1.1.1 through the `colab` CLI: fed identical features the embedding network matches `ECAPA_TDNN` bit for bit, an exact 0.0, on a full length clip and a padded one alike; the filterbank matches `Fbank` plus sentence `InputNormalization` to 2.9e-06; `encode_batch` end to end to 3.2e-05 (cosine 0.99999994), and to 4.2e-05 on a padded batch of two; `classify_batch` logits to 2.2e-07 with identical top-1 on every clip. All 231 keys map with no MISSING, no UNEXPECTED and no shape mismatch. On 40 LibriSpeech `validation.clean` clips from 8 speakers, cosine similarity is 0.7305+-0.1426 within a speaker against 0.1317+-0.0995 across, an equal error rate of 0.0250 on those 780 pairs, matching upstream to four decimals. speechbrain is not a dependency: `Conv1d`'s reflect padding, `BatchNorm1d`, `Fbank`, `Filterbank`, `STFT` and `InputNormalization` are all inlined. Not inherited from `spark_tts_bicodec`'s `SparkTTSEcapaTdnn`, which is a different implementation of the paper with incompatible weights; see that folder's README Lineage section. |
...
| Parler-TTS | Yes | Yes | Yes | The bundled codec is not converted at all: all 301 `audio_encoder.model.*` tensors of `parler-tts/parler-tts-mini-v1` are bit for bit those of `parler-tts/dac_44khZ_8kbps`, the repository its own `audio_encoder._name_or_path` names, and upstream's `freeze_encoders` freezes the codec unconditionally, so the codec is loaded from `descript/dac_44khz`, which publishes the same weights in `DacModel`'s layout. Folding the bundle by hand instead reproduces that repository over 223 tensors, 145 bit identical and the 78 weight-norm-folded convolutions agreeing to 2.4e-07, which flips 1.2 and 0.7 percent of the DAC codes of the two `.cache/verify/assets` clips and leaves the round trip unchanged to 0.01 dB. What is checked at load time is the shape and bandwidth of the checkpoint's `audio_encoder` entry against the repository's configuration, and the 45 folded quantizer tensors against the bundle, 27 of them bit identical and the 18 projections agreeing to 5.96e-08 against a 1e-06 tolerance. Round trip over those two clips resampled to 44.1 kHz, encode then decode through `DacModel`, SNR 11.50 and 8.75 dB (SI-SDR 11.27 and 8.22), reproducing the 11.46 and 8.82 recorded before to 0.07 dB. Negative control, the 36 quantizer projection tensors redrawn from a unit normal at seed 0: SNR -8.67 and -17.34 dB. `992d149c` replaced the vendored `descript-audio-codec` with native `DacModel`. `b58b94cf` added the missing `ParlerTTSProcessor` and declared `sub_configs`, without which transformers 5 skipped the decoder during dtype resolution and half-precision loads crashed. Loads at 0 MISSING and 0 UNEXPECTED in float32 and float16, and three seeds transcribe "Hey, how are you doing today?" verbatim under `whisper-large-v3-turbo`, with `wav2vec2-base-960h` verbatim on two of the three and garbling the first two words of seed 1. |
...
| Qwen3-TTS | Yes | Yes | Yes | Import relays plus a task-dispatching processor. `af7968ea` fixed voice design running in streaming mode: 17 of 17 prompts now transcribe verbatim. `encoder.upsample.conv.weight` used to report MISSING; the backport closed it, and a fresh conversion loads at 0 MISSING and 0 UNEXPECTED. The `speech_tokenizer` subfolder is now read through a conversion mapping registered for `Qwen3TTSTokenizerMultiCodebookConfig` rather than through the vendored conversion script, which also stops a `speech_tokenizer_converted` directory being written inside the downloaded snapshot. The tokenizer it builds matches the old conversion bit for bit on all 496 weight tensors, differing only on 16 encoder `initialized` flags that the script forced to 1.0 and no code reads, and the same codes decode to identical waveforms across three seeds, which transcribe verbatim under `whisper-large-v3-turbo` and `wav2vec2-base-960h`. See above. |
...
| UTMOSv2 (`utmos_v2`) | Yes | Yes | n/a (never vendored; entered as a pip dependency in `23433734`, removed in `6b407586`) | A mean opinion score predictor, not a synthesizer, so section 2.5's transcription standard does not apply and the numeric check against upstream's own classes is the evidence. On `fold0_s42_best_model.pth` through the `colab` CLI on an L4: logits agree with upstream `SSLMultiSpecExtModelV2` to 4.6e-06 absolute (0.8902318 against 0.8902364) on identical inputs, the mel front end agrees with `librosa.feature.melspectrogram` to 8.5e-06 across all four window lengths, the resize agrees with `torchvision.transforms.Resize` to 7.9e-06, and `remove_silent_sections` is byte identical to upstream's on real clips. End to end on six real clips, fold 0, 64 draws per estimate and three estimates per clip per side, the per-clip difference against upstream's `predict` has mean -0.0054 MOS and largest magnitude 0.0329 against a repeat spread of 0.010 to 0.033, so the two agree inside the estimator's own noise. The draw count is the finding: a first attempt at sixteen draws showed a gap of up to 0.28 that read as a systematic bias and was not one, since a rerun moved upstream's own estimate for one clip from 3.823 to 3.615. Sixteen draws does not resolve 0.1 MOS and no comparison at that sample size means anything. All 3343 checkpoint keys map with no MISSING, no UNEXPECTED and no shape mismatch, and one predictor's 203.68M parameters equal timm's `tf_efficientnetv2_s` less its classifier, four times over, plus wav2vec 2.0. timm is inlined rather than depended on: `EFFICIENTNET_V2_S_STAGES` plus five modules replace `_gen_efficientnetv2_s` and `SelectAdaptivePool2d`, so the folder adds no dependency. Only `fusion_stage3` is carried over, which is the only configuration with published weights. |
...
| UTMOSv2 decoupling | Done | | | Superseded by the `utmos_v2` model folder above, which reimplements the predictor rather than wrapping the `utmosv2` package. The package dependency and its `eval` entry were removed in `6b407586`; nothing reintroduces them. |
