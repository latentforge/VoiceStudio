...
`language` selects the pronunciation tendency, `"en"` or `"zh"`. The three guidance scales are independent: one pushes the semantic span away from the branch that drops the instruction, and two push the first codebook away from the branches that drop the instruction and the semantic span. Each scale left at `1.0` saves one forward pass per decoding step. The values above are the ones `infer.sh` uses upstream.
...
- **The Vocos vocoder.** `infer.sh` runs with `--vocoder vocos`, decoding the eight EnCodec codebooks through `vocos-encodec-24khz`, a ConvNeXt backbone with an iSTFT head over the codebook embeddings. `transformers-tts` ships no Vocos, so `generate` uses the EnCodec decoder, which is upstream's own `--vocoder encodec` alternative. The released `vocos-encodec-24khz/pytorch_model.bin` is left unconverted, and generated audio carries the EnCodec decoder's quality rather than Vocos's. Restoring it means either adding the `vocos` package or porting the backbone and head into this folder.
...
- `voicestudio/models/__init__.py` needs a `from .vox_instruct import *` line.
- `PROJECT.md` needs a VoxInstruct status entry carrying the gaps listed above.
- Nothing in `pyproject.toml` or `uv.lock` changes. The migration removes `fairseq`, `encodec`, `vocos`, `peft`, `einops`, `matplotlib`, `scipy`, `accelerate`, `flash-attn`, `sentencepiece` and `protobuf` from what this model needs. What remains is `torch`, `torchaudio`, `transformers`, `numpy`, `huggingface_hub`, and, at conversion time only, `joblib` and `scikit-learn` to read the k-means codebook, all of which the project already installs.
