...
class OmniVoiceGenerationConfig(GenerationConfig):
    r"""
    A [`GenerationConfig`] parameterized for [`OmniVoiceGenerationMixin.generate`]. OmniVoice decodes a fixed-length
    ...
    Args:
        ...
        guidance_scale (`float`, *optional*, defaults to 2.0):
            Classifier-free guidance scale. `0` disables guidance and skips the unconditional forward pass.
...
def _get_time_steps(num_step: int, t_shift: float, device: torch.device) -> torch.Tensor:
    timesteps = torch.linspace(0.0, 1.0, num_step + 1, device=device)
...
class OmniVoiceGenerationMixin:
    ...
    def _unmasking_schedule(self, target_length: int, generation_config: OmniVoiceGenerationConfig) -> list[int]:
        ...
        timesteps = _get_time_steps(
            generation_config.num_step, generation_config.t_shift, device=torch.device("cpu")
        ).tolist()
        ...
        for step in range(generation_config.num_step):
            if step == generation_config.num_step - 1:
                ...
                num = min(math.ceil(total * (timesteps[step + 1] - timesteps[step])), remaining)
    ...
    def _predict_tokens_with_scoring(
        ...
        """
        Applies classifier-free guidance and picks a candidate code per position.
        ...
        if generation_config.class_temperature > 0.0:
            ...
            predicted_tokens = _gumbel_sample(filtered_probs, generation_config.class_temperature).argmax(dim=-1)
        else:
        ...
        **kwargs,
        ...
        Args:
            ...
            generation_config ([`OmniVoiceGenerationConfig`], *optional*):
                Decoding parameters. Defaults to a fresh [`OmniVoiceGenerationConfig`].
            kwargs:
        ...
        Returns:
            ...
            codes. Frames past an item's own `target_lengths` stay at `config.audio_mask_id` and are trimmed by
            [`~OmniVoiceProcessor.batch_decode`].
        """
        ...
        if attention_mask is None:
            prompt_lengths = [seq_length] * batch_size
        else:
            prompt_lengths = attention_mask.to(device).long().sum(-1).tolist()
        target_lengths = [int(length) for length in target_lengths]

        # Rows `[0, batch_size)` hold the conditional sequences, rows `[batch_size, 2 * batch_size)` the canvas-only
        # unconditional counterparts, so a single forward pass covers both branches of the guidance.
        batch_input_ids = torch.full(
            (2 * batch_size, num_codebooks, seq_length), mask_id, dtype=torch.long, device=device
        )
        batch_audio_mask = torch.zeros((2 * batch_size, seq_length), dtype=torch.bool, device=device)
        batch_attention_mask = torch.zeros(
            (2 * batch_size, 1, seq_length, seq_length), dtype=torch.bool, device=device
        )
        ...
        for i in range(batch_size):
            ...
            batch_attention_mask[i, :, :prompt_length, :prompt_length] = True

            batch_input_ids[batch_size + i, :, :target_length] = input_ids[
                i, :, prompt_length - target_length : prompt_length
            ]
            batch_audio_mask[batch_size + i, :target_length] = audio_mask[
                i, prompt_length - target_length : prompt_length
            ]
            batch_attention_mask[batch_size + i, :, :target_length, :target_length] = True
            if seq_length > target_length:
                # Padding positions attend to themselves only, so their attention rows are never fully masked.
                padding = torch.arange(target_length, seq_length, device=device)
                batch_attention_mask[batch_size + i, :, padding, padding] = True
        ...
        for step in range(generation_config.num_step):
            ...
            for i in range(batch_size):
                ...
                prompt_length, target_length = prompt_lengths[i], target_lengths[i]
                conditional_logits = batch_logits[i : i + 1, :, prompt_length - target_length : prompt_length, :]
                ...
                batch_input_ids[i : i + 1, :, prompt_length - target_length : prompt_length] = committed
