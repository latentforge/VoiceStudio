...
# Full scale of a 16 bit sample, the reference amplitude the silence thresholds below are expressed against.
_FULL_SCALE = 32768.0


def _duration_ms(num_samples: int, sampling_rate: int) -> int:
    return round(num_samples / sampling_rate * 1000)


def _sample_index(milliseconds: int, sampling_rate: int) -> int:
    return int(milliseconds * sampling_rate / 1000)


def _squared_amplitude_prefix(audio: np.ndarray) -> np.ndarray:
    """Cumulative sum, over time, of the squared 16 bit sample values of every channel."""
    quantized = np.clip(audio * _FULL_SCALE, -_FULL_SCALE, _FULL_SCALE - 1).astype(np.int16).astype(np.float64)
    return np.concatenate([[0.0], np.cumsum(np.square(quantized).sum(axis=0))])


def _window_rms(
    prefix: np.ndarray, channels: int, num_samples: int, start_ms: int, end_ms: int, sampling_rate: int
) -> float:
    """Root mean square of the `[start_ms, end_ms)` window, on the 16 bit scale.

    A window reaching past the last sample is zero filled up to its nominal length, so its mean is taken over the
    requested number of samples rather than over the available ones.
    """
    duration = _duration_ms(num_samples, sampling_rate)
    start = _sample_index(min(start_ms, duration), sampling_rate)
    end = _sample_index(min(end_ms, duration), sampling_rate)
    count = (end - start) * channels
    if count <= 0:
        return 0.0
    total = prefix[min(end, num_samples)] - prefix[min(start, num_samples)]
    return math.floor(math.sqrt(total / count))


def _detect_silence(
    audio: np.ndarray, sampling_rate: int, min_silence_len: int, silence_thresh: float, seek_step: int
) -> list[list[int]]:
    """Millisecond ranges of `audio` that hold nothing louder than `silence_thresh` dBFS for `min_silence_len` ms."""
    ...
    prefix = _squared_amplitude_prefix(audio)

    last_start = duration - min_silence_len
    window_starts = list(range(0, last_start + 1, seek_step))
    ...
    silence_starts = [
        ...
        if _window_rms(prefix, channels, num_samples, start, start + min_silence_len, sampling_rate) <= threshold
    ]
    ...
    for start in silence_starts[1:]:
        # Two blips can leave a single window above the threshold in the middle of one long silence, so windows
        # that still overlap the running range are folded into it rather than starting a new one.
        if start != previous + seek_step and start > previous + min_silence_len:
            silent_ranges.append([range_start, previous + min_silence_len])
            range_start = start
        previous = start
    silent_ranges.append([range_start, previous + min_silence_len])
...
def _detect_nonsilent(
    ...
    """Millisecond ranges of `audio` that are the complement of [`_detect_silence`]."""
    duration = _duration_ms(audio.shape[-1], sampling_rate)
    silent_ranges = _detect_silence(audio, sampling_rate, min_silence_len, silence_thresh, seek_step)
    if not silent_ranges:
        return [[0, duration]]
    if silent_ranges[0] == [0, duration]:
        return []
    ...
    if silent_ranges[-1][1] != duration:
        nonsilent_ranges.append([previous_end, duration])
...
def _split_on_silence(
    ...
    """Sample ranges of the speech segments of `audio`, each padded by up to `keep_silence` ms of silence."""
    duration = _duration_ms(audio.shape[-1], sampling_rate)
    ranges = [
        [start - keep_silence, end + keep_silence]
        for start, end in _detect_nonsilent(audio, sampling_rate, min_silence_len, silence_thresh, seek_step)
    ]
    # A silence shorter than twice `keep_silence` would otherwise be emitted by both of its neighbours.
    ...
    return [
        (
            _sample_index(max(start, 0), sampling_rate),
            _sample_index(min(end, duration), sampling_rate),
        )
...
def _detect_leading_silence(
    audio: np.ndarray, sampling_rate: int, silence_thresh: float, chunk_size: int = 10
) -> int:
    """Millisecond at which the leading silence of `audio` ends."""
    channels, num_samples = audio.shape
    duration = _duration_ms(num_samples, sampling_rate)
    prefix = _squared_amplitude_prefix(audio)
    threshold = 10 ** (silence_thresh / 20) * _FULL_SCALE
    ...
    while (
        trimmed < duration
        and _window_rms(prefix, channels, num_samples, trimmed, trimmed + chunk_size, sampling_rate) < threshold
    ):
        trimmed += chunk_size
    return min(trimmed, duration)


def _trim_silence_edges(
    audio: np.ndarray, sampling_rate: int, lead_sil: int, trail_sil: int, silence_thresh: float
) -> np.ndarray:
    leading = max(0, _detect_leading_silence(audio, sampling_rate, silence_thresh) - lead_sil)
    audio = audio[..., _sample_index(leading, sampling_rate) :]

    reversed_audio = np.ascontiguousarray(audio[..., ::-1])
    trailing = max(0, _detect_leading_silence(reversed_audio, sampling_rate, silence_thresh) - trail_sil)
    reversed_audio = reversed_audio[..., _sample_index(trailing, sampling_rate) :]
...
def _remove_silence(
    ...
    """
    Drops the silences of `audio` that run longer than `mid_sil` ms and shortens the ones at its two edges.

    Args:
        audio (`np.ndarray` of shape `(channels, num_samples)`):
            Waveform to shorten.
        sampling_rate (`int`):
            Sampling rate of `audio`, in Hz.
        mid_sil (`int`, *optional*, defaults to 300):
            Length, in ms, above which an interior silence is dropped, and the amount of it kept on either side.
            `0` leaves interior silences alone.
        lead_sil (`int`, *optional*, defaults to 100):
            Leading silence kept, in ms.
        trail_sil (`int`, *optional*, defaults to 300):
            Trailing silence kept, in ms.
        silence_thresh (`float`, *optional*, defaults to -50.0):
            Level, in dBFS, below which a window counts as silent.

    Returns:
        `np.ndarray` of shape `(channels, num_samples)`: The shortened waveform.
    """
    if audio.shape[-1] == 0:
        return audio

    if mid_sil > 0:
        segments = _split_on_silence(audio, sampling_rate, mid_sil, silence_thresh, mid_sil, seek_step=10)
        if segments:
            audio = np.concatenate([audio[..., start:end] for start, end in segments], axis=-1)
        else:
            audio = audio[..., :0]
        if audio.shape[-1] == 0:
            return audio

    return _trim_silence_edges(audio, sampling_rate, lead_sil, trail_sil, silence_thresh)
...
def _trim_long_audio(
    ...
    """
    Cuts `audio` down to at most `max_duration` seconds at the last speech boundary that fits.
    ...
    Args:
        ...
        sampling_rate (`int`):
            Sampling rate of `audio`, in Hz.
        max_duration (`float`, *optional*, defaults to 15.0):
            Length, in seconds, the result is cut down to.
        min_duration (`float`, *optional*, defaults to 3.0):
            Length, in seconds, below which the cut is moved to `max_duration` instead of to a speech boundary.
        trim_threshold (`float`, *optional*, defaults to 20.0):
            Length, in seconds, below which `audio` is returned untouched.
    ...
    speech_ranges = _detect_nonsilent(audio, sampling_rate, min_silence_len=100, silence_thresh=-40.0, seek_step=10)
    ...
    for start, end in speech_ranges:
        if split < start <= max_ms:
            split = start
        if end > max_ms:
            break
    if split < min_ms:
        split = min(max_ms, _duration_ms(audio.shape[-1], sampling_rate))
    return audio[..., : _sample_index(split, sampling_rate)]
...
class OmniVoiceProcessor(ProcessorMixin):
    ...
    Args:
        ...
        asr_model_id (`str`, *optional*, defaults to `"openai/whisper-large-v3-turbo"`):
            Repository id of the speech recognition model [`~OmniVoiceProcessor.transcribe`] loads to fill in a
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, audio_tokenizer_id: str | None = None, **kwargs):
        r"""
        ...
        Args:
            ...
            audio_tokenizer_id (`str`, *optional*):
                Repository id of the [`HiggsAudioV2TokenizerModel`] to fall back to when the checkpoint does not
                bundle its own `audio_tokenizer` subfolder. Left unset, the fallback is taken from the model
                config's `audio_tokenizer_id`, and from `"eustlb/higgs-audio-v2-tokenizer"` when the path holds
                no model config either.
            kwargs:
        ...
        try:
            ...
            if audio_tokenizer_id is None:
                audio_tokenizer_id = cls._config_audio_tokenizer_id(pretrained_model_name_or_path, **kwargs)
            logger.warning_once(
    ...
    @staticmethod
    def _config_audio_tokenizer_id(pretrained_model_name_or_path, **kwargs) -> str:
        """Reads `audio_tokenizer_id` off the model config, which a processor-only path need not carry."""
        from transformers import AutoConfig

        try:
            config = AutoConfig.from_pretrained(pretrained_model_name_or_path, **kwargs)
        except (OSError, ValueError):
            return DEFAULT_AUDIO_TOKENIZER_ID
        return getattr(config, "audio_tokenizer_id", None) or DEFAULT_AUDIO_TOKENIZER_ID
    ...
    def preprocess_reference_audio(
        self, audio, sampling_rate: int | None = None, trim_long: bool = False
    ) -> torch.Tensor:
        """
        Brings a reference waveform into the shape the audio tokenizer expects: mono, at the processor's sampling
        rate, loud enough to encode, and without the silences that would be cloned along with the voice.

        Args:
            audio (`np.ndarray` or `torch.Tensor` of shape `(num_samples,)`, `(1, num_samples)` or `(channels, num_samples)`):
                Waveform to prepare. Multi-channel input is averaged down to mono.
            sampling_rate (`int`, *optional*):
                Sampling rate of `audio`. Defaults to the processor's own sampling rate; anything else is
                resampled.
            trim_long (`bool`, *optional*, defaults to `False`):
                Whether a clip longer than 20s may be cut at a speech boundary. Only safe when the transcript is
                derived from the result, since cutting invalidates a transcript supplied for the whole clip.

        Returns:
            `torch.Tensor` of shape `(1, num_samples)`: The prepared waveform.

        Raises:
            ValueError: If nothing is left of `audio` once its silences are removed.
        """
        waveform = self._boost_quiet(self._to_mono_tensor(audio, sampling_rate))
        array = waveform.numpy()
        if trim_long:
            array = _trim_long_audio(array, self.sampling_rate)
        array = _remove_silence(array, self.sampling_rate, mid_sil=200, lead_sil=100, trail_sil=200)
        if array.shape[-1] == 0:
            raise ValueError(
                "The reference audio holds no speech once its silences are removed. Pass "
                "`preprocess_reference=False` to encode it as it is."
            )
        return torch.from_numpy(array)

    def transcribe(self, audio, sampling_rate: int | None = None) -> str:
        """
        Transcribes a reference waveform with the speech recognition model named by `asr_model_id`, which is
        loaded on first use and then kept.

        Args:
            audio (`np.ndarray` or `torch.Tensor` of shape `(num_samples,)`, `(1, num_samples)` or `(channels, num_samples)`):
                Waveform to transcribe.
            sampling_rate (`int`, *optional*):
                Sampling rate of `audio`. Defaults to the processor's own sampling rate.
        ...
        if self._asr_pipeline is None:
            ...
            logger.warning_once(
                f"No `reference_text` was given, so '{self.asr_model_id}' is being loaded to transcribe the "
                f"reference audio. Pass `reference_text` to skip this."
            )
            self._asr_pipeline = pipeline("automatic-speech-recognition", model=self.asr_model_id)

        waveform = self._to_mono_tensor(audio, sampling_rate).squeeze(0).numpy()
        return self._asr_pipeline({"array": waveform, "sampling_rate": self.sampling_rate})["text"].strip()

    def encode_audio(
        self, audio, sampling_rate: int | None = None, normalize_loudness: bool = True
    ) -> torch.LongTensor:
        """
        ...
        Args:
            ...
            sampling_rate (`int`, *optional*):
                Sampling rate of `audio`. Defaults to the processor's own sampling rate; anything else is
                resampled.
            normalize_loudness (`bool`, *optional*, defaults to `True`):
                Whether a quiet waveform is raised to an RMS of `0.1` first. Set it to `False` for a waveform
                [`~OmniVoiceProcessor.preprocess_reference_audio`] already returned, which is raised there, on the
                loudness it had before its silences were dropped.
        ...
        if normalize_loudness:
            waveform = self._boost_quiet(waveform)
    ...
    @staticmethod
    def _boost_quiet(waveform: torch.Tensor) -> torch.Tensor:
        """Raises a quiet waveform to an RMS of 0.1, which [`~OmniVoiceProcessor.decode`] undoes on the output."""
        rms = float(waveform.pow(2).mean().sqrt())
        if 0 < rms < 0.1:
            return waveform * 0.1 / rms
        return waveform
    ...
    def __call__(
        ...
        Args:
            ...
            reference_text (`str` or `list[str]`, *optional*):
                Transcript of the reference audio. When it is missing and `reference_audio` is given, it is filled
                in by [`~OmniVoiceProcessor.transcribe`].
            ...
            preprocess_reference (`bool`, *optional*, defaults to `True`):
                Whether `reference_audio` is passed through [`~OmniVoiceProcessor.preprocess_reference_audio`]
        ...
        if reference_audio_codes is not None:
            ...
            reference_codes = []
            derived_texts = []
            for i, item in enumerate(reference_audios):
                item_text = reference_texts[i] if i < len(reference_texts) else None
                item_sampling_rate = sampling_rate
                if preprocess_reference:
                    # A clip is only shortened when its transcript is derived from the clip itself; shortening one
                    # the caller transcribed in full would leave audio and transcript out of step.
                    item = self.preprocess_reference_audio(item, sampling_rate, trim_long=item_text is None)
                    item_sampling_rate = None
                if item_text is None:
                    item_text = self.transcribe(item, item_sampling_rate)
                derived_texts.append(item_text)
                reference_codes.append(
                    self.encode_audio(item, item_sampling_rate, normalize_loudness=not preprocess_reference)
                )
            reference_codes = self._broadcast_tensor(reference_codes, batch_size)
            reference_texts = self._broadcast(derived_texts, batch_size)
        else:
    ...
    def decode(
        ...
        Args:
            ...
            sampling_rate (`int`, *optional*):
                Sampling rate of `reference_audio`.
            postprocess (`bool`, *optional*, defaults to `True`):
                Whether the long silences the model sometimes leaves between sentences are removed.
        ...
        if postprocess:
            waveform = _remove_silence(waveform, self.sampling_rate, mid_sil=500, lead_sil=100, trail_sil=100)
