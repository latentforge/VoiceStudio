...
class OmniVoiceConfig(PreTrainedConfig):
    ...
    used to instantiate an OmniVoice model according to the specified arguments, defining a decoder-only language
    model backbone (a [`Qwen3Model`] in the released checkpoint) paired with a fused multi-codebook audio embedding
    and output head. Audio frames are predicted by iterative unmasking rather than autoregressively, so the backbone
    attends bidirectionally and never uses a cache.
    ...
    Args:
        ...
        audio_tokenizer_id (`str`, *optional*, defaults to `"eustlb/higgs-audio-v2-tokenizer"`):
            Repository id of the [`HiggsAudioV2TokenizerModel`] used by [`OmniVoiceProcessor`] when a checkpoint does
            not bundle its own `audio_tokenizer` subfolder.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.audio_codebook_weights = (
            audio_codebook_weights if audio_codebook_weights is not None else [8, 8, 6, 6, 4, 4, 2, 2]
        )
        if len(self.audio_codebook_weights) != num_audio_codebook:
            raise ValueError(
                f"`audio_codebook_weights` holds {len(self.audio_codebook_weights)} weights but "
                f"`num_audio_codebook` is {num_audio_codebook}; one weight per codebook is required."
            )
