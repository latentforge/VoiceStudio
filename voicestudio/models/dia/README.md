...
```python
from voicestudio.models.dia import DiaForConditionalGeneration, DiaProcessor
...
The `Auto` classes reach the same two objects. Dia ships in `transformers` itself, so this route needs
no registration from this repository and works without importing `voicestudio.models` first:
