...
def _estimator_attn_bias(mask: torch.Tensor) -> torch.Tensor | None:
    """Builds the additive attention bias `CosyVoiceV1EstimatorBlock` expects from a `(batch_size, 1,
    sequence_length)` padding mask, mirroring the original CosyVoice `mask_to_bias` helper."""
    if mask.all():
        return None
    bias = (~mask.squeeze(1).bool())[:, None, None, :].to(mask.dtype) * torch.finfo(mask.dtype).min
    return bias
...
class CosyVoiceV1EstimatorAttention(nn.Module):
    """Self-attention with separate `to_q`/`to_k`/`to_v` (unbiased) projections and a biased `to_out`
    projection, matching `diffusers.models.attention_processor.Attention` as instantiated by
    `matcha.models.components.transformer.BasicTransformerBlock`'s `attn1`."""
    ...
    def __init__(self, dim: int, num_heads: int, head_dim: int, dropout: float):
        ...
        self.to_out = nn.ModuleList([nn.Linear(inner_dim, dim), nn.Dropout(dropout)])

    def forward(self, hidden_states: torch.Tensor, attn_bias: torch.Tensor | None) -> torch.Tensor:
        ...
        attn_out = F.scaled_dot_product_attention(query, key, value, attn_mask=attn_bias)
        attn_out = attn_out.transpose(1, 2).reshape(batch_size, seq_len, self.num_heads * self.head_dim)
        return self.to_out[1](self.to_out[0](attn_out))


class CosyVoiceV1EstimatorGELU(nn.Module):
    """Matches `diffusers.models.attention.GELU`: a single `Linear` projection followed by `GELU`, as opposed to
    `GEGLU`'s gated two-way split of a doubled-width projection."""
...
class CosyVoiceV1EstimatorFeedForward(nn.Module):
    """Plain (non-gated) GELU feed-forward, matching `diffusers.models.attention.FeedForward` with
    `activation_fn="gelu"`: a single `GELU(dim, inner_dim)` projection (`net.0.proj`) followed by dropout and an
    output projection (`net.2`), with no second (gate) branch."""

    def __init__(self, dim: int, mult: int, dropout: float):
        super().__init__()
        inner_dim = dim * mult
        self.net = nn.ModuleList(
            [CosyVoiceV1EstimatorGELU(dim, inner_dim), nn.Dropout(dropout), nn.Linear(inner_dim, dim)]
        )
...
class CosyVoiceV1EstimatorBlock(nn.Module):
    """`matcha.models.components.transformer.BasicTransformerBlock` as instantiated by CosyVoice's flow decoder
    (no `num_embeds_ada_norm`/`norm_type` override, so `norm1`/`norm3` are plain `LayerNorm`, not AdaLN, and the
    flow-matching timestep is not injected into this block at all, only into the surrounding
    [`CosyVoiceV1ResnetBlock1D`] via FiLM). `cross_attention_dim` and `double_self_attention` are both unset, so
    there is no `attn2`/`norm2` cross-attention branch."""
    ...
    def __init__(self, dim: int, num_heads: int, head_dim: int, dropout: float):
        ...
        self.ff = CosyVoiceV1EstimatorFeedForward(dim, mult=4, dropout=dropout)

    def forward(self, hidden_states: torch.Tensor, attn_bias: torch.Tensor | None, time_emb: torch.Tensor) -> torch.Tensor:
        hidden_states = hidden_states + self.attn1(self.norm1(hidden_states), attn_bias)
...
class CosyVoiceV1ConditionalDecoder(nn.Module):
    ...
    def __init__(self, config: CosyVoiceV1FlowConfig):
        super().__init__()
        in_channels = config.output_size * 4
        ...
        def make_stage(input_channel, output_channel):
            ...
            blocks = nn.ModuleList(
                [
                    CosyVoiceV1EstimatorBlock(output_channel, config.decoder_num_heads, config.decoder_attention_head_dim, config.decoder_dropout)
        ...
        for i, ch in enumerate(channels):
            ...
            downsample = nn.Conv1d(ch, ch, 3, stride=1, padding=1) if is_last else CosyVoiceV1Downsample1D(ch)
        ...
        for i in range(len(up_channels) - 1):
            ...
            upsample = (
                ...
                if is_last
                else CosyVoiceV1Upsample1D(up_channels[i + 1])
            )
    ...
    def forward(self, x, mask, mu, t, spks, cond) -> torch.Tensor:
        ...
        hidden_states = torch.cat([x, mu, spk_expanded, cond], dim=1)
        ...
        for resnet, blocks, downsample in self.down_blocks:
            ...
            attn_bias = _estimator_attn_bias(m)
            hidden_states_t = hidden_states.transpose(1, 2)
            for block in blocks:
                hidden_states_t = block(hidden_states_t, attn_bias, time_emb)
...
class CosyVoiceV1HiFTGenerator(CosyVoiceV1PreTrainedModel):
    ...
    def _istft(self, magnitude: torch.Tensor, phase: torch.Tensor) -> torch.Tensor:
        ...
        return torch.istft(complex_spec, self.istft_n_fft, self.istft_hop_len, self.istft_n_fft, window=window)
