...
def _filter_for_decimate(x, ratio):
    a, b = _DECIMATE_COEFFICIENTS[ratio]
    return _direct_form_2(x, a, (b[0], b[1], b[1], b[0]))


def _decimate(x, ratio):
    n_fact = 9
    x_length = x.shape[0]
    ...
    filtered = _filter_for_decimate(padded, ratio)
    filtered = _filter_for_decimate(filtered[::-1].copy(), ratio)
...
def _zero_crossing_engine(signal, length, fs):
...
def _four_zero_crossing_intervals(signal, length, fs):
    negative = _zero_crossing_engine(signal, length, fs)
    inverted = -signal
    positive = _zero_crossing_engine(inverted, length, fs)
    differentiated = inverted[: length - 1] - inverted[1:length]
    peak = _zero_crossing_engine(differentiated, length - 1, fs)
    dip = _zero_crossing_engine(-differentiated, length - 1, fs)
    return negative, positive, peak, dip


def _filtered_signal(boundary_f0, fft_size, fs, spectrum, y_length):
    """Convolves the analysed spectrum with a Nuttall-windowed band-pass filter."""
    filter_length_half = _matlab_round(fs / boundary_f0 * 2.0)
...
def _f0_candidate_contour(crossings, boundary_f0, f0_floor, f0_ceil, temporal_positions):
    ...
    for locations, intervals in crossings:
        candidate += _interp1(locations, intervals, temporal_positions)
    candidate /= 4.0
...
def _raw_f0_candidates(boundary_f0_list, actual_fs, y_length, temporal_positions, spectrum, fft_size, f0_floor, f0_ceil):
    ...
    for band, boundary_f0 in enumerate(boundary_f0_list):
        signal = _filtered_signal(boundary_f0, fft_size, actual_fs, spectrum, y_length)
        crossings = _four_zero_crossing_intervals(signal, y_length, actual_fs)
        raw[band] = _f0_candidate_contour(crossings, boundary_f0, f0_floor, f0_ceil, temporal_positions)
    return raw


def _detect_official_f0_candidates(raw, max_candidates):
...
def _overlap_f0_candidates(candidates, number_of_candidates):
...
def _refine_f0_batch(x, fs, positions, f0s, half_window_length, fft_size, f0_floor, f0_ceil):
    ...
    basic_index = _matlab_round_array((positions - half_window_length / fs) * fs + 0.001)
    ...
    index = _matlab_round_array(f0s[:, None] * fft_size / fs * harmonics[None, :])
    ...
    rejected = (refined < f0_floor) | (refined > f0_ceil) | (score < 2.5)
    refined[rejected] = 0.0
    score[rejected] = 0.0
    return refined, score


def _refine_f0_candidates(x, fs, temporal_positions, candidates, f0_floor, f0_ceil):
    ...
    for half_window_length in np.unique(half_window_lengths):
        ...
        for start in range(0, selected.shape[0], chunk):
            ...
            refined, score = _refine_f0_batch(
                x, fs, positions[rows], f0s[rows], int(half_window_length), fft_size, f0_floor, f0_ceil
            )
...
def _remove_unreliable_candidates(candidates, scores):
...
def _search_f0_base(candidates, scores):
...
def _fix_step_1(f0_base, allowed_range):
...
def _fix_step_2(f0_step1, voice_range_minimum):
    f0_step2 = f0_step1.copy()
    boundaries = _boundary_list(f0_step1)
...
def _multi_channel_f0(f0, boundaries):
...
def _select_best_f0(reference_f0, candidates, allowed_range):
    best_f0 = 0.0
...
def _extend_f0(origin, last_point, shift, candidates, allowed_range, extended_f0):
    threshold = 4
    ...
    for step in range(abs(last_point - origin) + 1):
        ...
        extended_f0[target], _ = _select_best_f0(tmp_f0, candidates[target], allowed_range)
...
def _extend(channels, f0_length, boundaries, candidates, allowed_range):
    ...
    for section in range(sections):
        ...
        boundaries[section * 2] = _extend_f0(
...
def _make_sorted_order(boundaries, sections):
...
def _search_score(f0, candidates, scores):
...
def _merge_f0_sub(merged, start1, end1, channel, start2, end2, candidates, scores):
    if start1 <= start2 and end1 >= end2:
        return end1

    span = slice(start2, end1 + 1)
    score1 = sum(
        _search_score(merged[i], candidates[i], scores[i]) for i in range(start2, end1 + 1)
    )
    score2 = sum(
        _search_score(channel[i], candidates[i], scores[i]) for i in range(start2, end1 + 1)
    )
    del span
    if score1 > score2:
...
def _merge_f0(channels, boundaries, sections, candidates, scores):
    order = _make_sorted_order(boundaries, sections)
...
def _fix_step_3(f0_step2, candidates, scores, allowed_range):
    f0_step3 = f0_step2.copy()
    f0_length = f0_step2.shape[0]
    boundaries = _boundary_list(f0_step2)
    if boundaries.shape[0] == 0:
        return f0_step3

    channels = _multi_channel_f0(f0_step2, boundaries)
    sections = _extend(channels, f0_length, boundaries, candidates, allowed_range)
    if sections != 0:
        f0_step3 = _merge_f0(channels, boundaries, sections, candidates, scores)
    return f0_step3


def _fix_step_4(f0_step3, threshold):
    f0_step4 = f0_step3.copy()
    boundaries = _boundary_list(f0_step3)
...
def _fix_f0_contour(candidates, scores):
    contour = _search_f0_base(candidates, scores)
    contour = _fix_step_1(contour, 0.008)
    contour = _fix_step_2(contour, 6)
    contour = _fix_step_3(contour, candidates, scores, 0.18)
    return _fix_step_4(contour, 9)


def _filtering_f0(x, start, end):
    x = x.copy()
    x[:start] = x[start]
    x[end + 1 :] = x[end]
    a = _SMOOTHING_A
    b = (_SMOOTHING_B[0], _SMOOTHING_B[1], _SMOOTHING_B[0])
    forward = _direct_form_2(x, a, b)[::-1].copy()
    return _direct_form_2(forward, a, b)[::-1].copy()


def _smooth_f0_contour(f0):
    lag = 300
    f0_length = f0.shape[0]
    contour = np.zeros(f0_length + lag * 2, dtype=np.float64)
    contour[lag : lag + f0_length] = f0

    boundaries = _boundary_list(contour)
    channels = _multi_channel_f0(contour, boundaries)

    smoothed = np.zeros(f0_length, dtype=np.float64)
    for section in range(boundaries.shape[0] // 2):
        start, end = boundaries[section * 2], boundaries[section * 2 + 1]
        filtered = _filtering_f0(channels[section], start, end)
        smoothed[start - lag : end + 1 - lag] = filtered[start : end + 1]
    return smoothed


def _samples_for_harvest(fs, x_length, frame_period):
    return int(1000.0 * x_length / fs / frame_period) + 1


def _waveform_and_spectrum(x, y_length, fft_size, decimation_ratio):
    ...
    if decimation_ratio == 1:
        ...
        decimated = _decimate(extended, decimation_ratio)
...
def _harvest_general_body(x, fs, frame_period, f0_floor, f0_ceil, channels_in_octave, speed):
    ...
    fft_size = _suitable_fft_size(y_length + 5 + 2 * int(2.0 * actual_fs / boundary_f0_list[0]))

    y, y_spectrum = _waveform_and_spectrum(x, y_length, fft_size, decimation_ratio)

    f0_length = _samples_for_harvest(fs, x_length, frame_period)
    temporal_positions = np.arange(f0_length, dtype=np.float64) * frame_period / 1000.0

    overlap_parameter = 7
    max_candidates = _matlab_round(number_of_channels / 10.0) * overlap_parameter

    raw = _raw_f0_candidates(
        boundary_f0_list, actual_fs, y_length, temporal_positions, y_spectrum, fft_size, f0_floor, f0_ceil
    )
    candidates, detected = _detect_official_f0_candidates(raw, max_candidates)
    candidates = _overlap_f0_candidates(candidates, detected)
    number_of_candidates = detected * overlap_parameter

    candidates = candidates[:, :number_of_candidates]
    candidates, scores = _refine_f0_candidates(
        y[:y_length], actual_fs, temporal_positions, candidates, f0_floor, f0_ceil
    )
    candidates, scores = _remove_unreliable_candidates(candidates, scores)

    contour = _fix_f0_contour(candidates, scores)
    return temporal_positions, _smooth_f0_contour(contour)
...
def _harvest(x, fs, f0_floor=71.0, f0_ceil=800.0, frame_period=5.0):
    ...
    Args:
        x (`np.ndarray`):
            Mono waveform, in double precision.
        fs (`int`):
            Sampling rate of `x`.
    ...
    dimension_ratio = _matlab_round(fs / 8000.0)

    if frame_period == 1.0:
        positions, f0 = _harvest_general_body(
            x, fs, 1, f0_floor, f0_ceil, channels_in_octave, dimension_ratio
        )
        return f0, positions

    _, basic_f0 = _harvest_general_body(x, fs, 1, f0_floor, f0_ceil, channels_in_octave, dimension_ratio)
    f0_length = _samples_for_harvest(fs, x.shape[0], frame_period)
    positions = np.arange(f0_length, dtype=np.float64) * frame_period / 1000.0
    index = np.minimum(basic_f0.shape[0] - 1, _matlab_round_array(positions * 1000.0))
    return basic_f0[index], positions


def _fix_f0_stonemask(power_spectrum, numerator_i, fft_size, fs, f0s, number_of_harmonics):
...
def _stonemask_batch(x, fs, positions, f0s, half_window_length, fft_size):
    width = half_window_length * 2 + 1
    window_length_in_time = width / fs

    offsets = np.arange(width, dtype=np.int64)
    base_time = (offsets[None, :] - half_window_length) / fs
    index_raw = _matlab_round_array((positions[:, None] + base_time) * fs)
    ...
    tentative = _fix_f0_stonemask(power_spectrum, numerator_i, fft_size, fs, f0s, 2)
    accepted = (tentative > 0.0) & (tentative <= f0s * 2)
    refined = np.zeros_like(f0s)
    if accepted.any():
        rows = np.flatnonzero(accepted)
        refined[rows] = _fix_f0_stonemask(
...
def _stonemask(x, fs, temporal_positions, f0):
    """
    ...
    for half_window_length in np.unique(half_window_lengths):
        ...
        for start in range(0, selected.shape[0], chunk):
            rows = selected[start : start + chunk]
            refined[frames[rows]] = _stonemask_batch(
                x, fs, temporal_positions[frames[rows]], f0s[rows], int(half_window_length), fft_size
            )
    return refined
...
def _design_low_cut_filter(taps, fft_size):
...
def _spectrum_for_estimation(x, y_length, actual_fs, fft_size, decimation_ratio):
    y = np.zeros(fft_size, dtype=np.float64)
    if decimation_ratio != 1:
        decimated = _decimate(x, decimation_ratio)
    ...
    cutoff_in_sample = _matlab_round(actual_fs / _CUT_OFF)
    low_cut_filter = _design_low_cut_filter(cutoff_in_sample * 2 + 1, fft_size)
    return spectrum * np.fft.rfft(low_cut_filter)


def _dio_filtered_signal(half_average_length, fft_size, spectrum, y_length):
...
def _dio_f0_candidate_contour(crossings, boundary_f0, f0_floor, f0_ceil, temporal_positions):
    f0_length = temporal_positions.shape[0]
    if any(locations.shape[0] < 3 for locations, _ in crossings):
        return np.zeros(f0_length, dtype=np.float64), np.full(f0_length, _MAXIMUM_VALUE)

    interpolated = np.stack(
        [_interp1(locations, intervals, temporal_positions) for locations, intervals in crossings]
    )
...
def _dio_candidates_and_scores(
    boundary_f0_list, actual_fs, y_length, temporal_positions, spectrum, fft_size, f0_floor, f0_ceil
):
    ...
    for band, boundary_f0 in enumerate(boundary_f0_list):
        signal = _dio_filtered_signal(
            _matlab_round(actual_fs / boundary_f0 / 2.0), fft_size, spectrum, y_length
        )
        crossings = _four_zero_crossing_intervals(signal, y_length, actual_fs)
        candidate, score = _dio_f0_candidate_contour(
            crossings, boundary_f0, f0_floor, f0_ceil, temporal_positions
        )
...
def _dio_best_f0_contour(candidates, scores):
    return candidates[np.argmin(scores, axis=0), np.arange(candidates.shape[1])]


def _dio_fix_step_1(best_f0_contour, voice_range_minimum, allowed_range):
...
def _dio_fix_step_2(f0_step1, voice_range_minimum):
    f0_step2 = f0_step1.copy()
...
def _voiced_section_edges(f0):
    onset = (f0[1:] != 0.0) & (f0[:-1] == 0.0)
    offset = (f0[1:] == 0.0) & (f0[:-1] != 0.0)
    return np.flatnonzero(onset) + 1, np.flatnonzero(offset)


def _dio_select_best_f0(current_f0, past_f0, candidates, target_index, allowed_range):
...
def _dio_fix_step_3(f0_step2, candidates, allowed_range, offsets):
    f0_length = f0_step2.shape[0]
    f0_step3 = f0_step2.copy()
    for index, start in enumerate(offsets):
        limit = f0_length - 1 if index == offsets.shape[0] - 1 else offsets[index + 1]
        for j in range(start, limit):
            f0_step3[j + 1] = _dio_select_best_f0(
...
def _dio_fix_step_4(f0_step3, candidates, allowed_range, onsets):
    ...
    for index in range(onsets.shape[0] - 1, -1, -1):
        ...
        for j in range(onsets[index], limit, -1):
            f0_step4[j - 1] = _dio_select_best_f0(
...
def _dio_fix_f0_contour(frame_period, candidates, best_f0_contour, f0_floor, allowed_range):
    f0_length = best_f0_contour.shape[0]
    voice_range_minimum = int(0.5 + 1000.0 / frame_period / f0_floor) * 2 + 1
    if f0_length <= voice_range_minimum:
        return np.zeros(f0_length, dtype=np.float64)

    contour = _dio_fix_step_1(best_f0_contour, voice_range_minimum, allowed_range)
    contour = _dio_fix_step_2(contour, voice_range_minimum)
    onsets, offsets = _voiced_section_edges(contour)
    contour = _dio_fix_step_3(contour, candidates, allowed_range, offsets)
    return _dio_fix_step_4(contour, candidates, allowed_range, onsets)
...
def _dio(
    ...
    Args:
        ...
        fs (`int`):
            Sampling rate of `x`.
        f0_floor (`float`, *optional*, defaults to 71.0):
            Lowest f0 the filter bank covers, in Hz.
        f0_ceil (`float`, *optional*, defaults to 800.0):
            Highest f0 the filter bank covers, in Hz.
    ...
    x = np.ascontiguousarray(x, dtype=np.float64)
    x_length = x.shape[0]
    number_of_bands = 1 + int(math.log(f0_ceil / f0_floor) / _LOG2 * channels_in_octave)
    boundary_f0_list = f0_floor * np.power(
    ...
    actual_fs = fs / decimation_ratio
    fft_size = _suitable_fft_size(
        y_length
        + _matlab_round(actual_fs / _CUT_OFF) * 2
        + 1
        + 4 * int(1.0 + actual_fs / boundary_f0_list[0] / 2.0)
    )

    spectrum = _spectrum_for_estimation(x, y_length, actual_fs, fft_size, decimation_ratio)

    f0_length = _samples_for_harvest(fs, x_length, frame_period)
    temporal_positions = np.arange(f0_length, dtype=np.float64) * frame_period / 1000.0

    candidates, scores = _dio_candidates_and_scores(
        boundary_f0_list,
        actual_fs,
        y_length,
        temporal_positions,
    ...
    best = _dio_best_f0_contour(candidates, scores)
    f0 = _dio_fix_f0_contour(frame_period, candidates, best, f0_floor, allowed_range)
...
class CosyVoiceV1Processor(ProcessorMixin):
    ...
    def compute_f0(
        ...
        contour, time_axis = _harvest(samples, target_rate, frame_period=frame_period)
        if (contour != 0).sum() < 5:
            contour, time_axis = _dio(samples, target_rate, frame_period=frame_period)
        contour = _stonemask(samples, target_rate, time_axis, contour)
