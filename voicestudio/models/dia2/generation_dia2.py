...
class Dia2TextStateMachine:
    ...
    def new_state(self, entries) -> Dia2TextStreamState:
        r"""
        Args:
...
def sample_from_logits(logits: torch.Tensor, temperature: float, top_k: int) -> torch.Tensor:
    ...
    # A row that softmaxed to all zeros would make `multinomial` raise; fall back to id 0 for it.
...
class Dia2GenerationMixin:
    ...
    def generate(
        ...
        Args:
            ...
            prefix_word_start_frames (`list[int]`, *optional*):
                Frame at which each prefix word starts, as returned by [`Dia2Processor`].
        ...
        if prefix_audio_codes is not None:
            prefix_codes = apply_delay_pattern(
                prefix_audio_codes[0].to(device), delay_pattern, config.audio_pad_token_id
            )
            num_prefix_frames = prefix_codes.shape[-1]

        total_frames = max(max_new_frames + num_prefix_frames + 1, max_new_frames)
        codes = torch.full(
            (branches, num_codebooks, total_frames), UNGENERATED_CODE, dtype=torch.long, device=device
        )
        if prefix_codes is not None:
            codes[:, :, :num_prefix_frames] = prefix_codes
        ...
        if prefix_codes is not None:
            start_frame, past_key_values = self._warm_up_prefix(
                frame_tokens, prefix_codes, delays, prefix_word_start_frames, machine, state
            )
        ...
        for offset in range(max_new_frames):
            ...
            codebook_logits = mask_audio_logits(
                codebook_logits, config.audio_pad_token_id, config.audio_bos_token_id
            )
            ...
            for codebook in range(num_codebooks - 1):
                ...
                depth_logits = mask_audio_logits(
                    depth_outputs.logits, config.audio_pad_token_id, config.audio_bos_token_id
                )
                if branches > 1:
        ...
        generated = torch.where(
            generated == UNGENERATED_CODE, torch.full_like(generated, config.audio_pad_token_id), generated
        )
        aligned = revert_delay_pattern(generated, delay_pattern, config.audio_pad_token_id)

        crop = 0 if keep_prefix_audio else max(first_word_frame or start_frame, 0)
    ...
    def _fill_codebook_channels(
        ...
        if frame < codes.shape[-1]:
            ...
            target.fill_(self.config.audio_bos_token_id)
        target.masked_fill_(delays.unsqueeze(0) > frame, self.config.audio_bos_token_id)
        target.masked_fill_(target == UNGENERATED_CODE, self.config.audio_bos_token_id)
    ...
    def _warm_up_prefix(
        ...
        num_frames = prefix_codes.shape[-1]
        past_key_values = None

        for frame in range(num_frames):
            target = frame_tokens[:, 0, 2:]
            target.copy_(prefix_codes[:, frame].unsqueeze(0).expand_as(target))
            target.masked_fill_(delays.unsqueeze(0) > frame, config.audio_bos_token_id)
            outputs = self.backbone_model(
                input_ids=frame_tokens,
                position_ids=torch.tensor([[frame]], device=device),
                past_key_values=past_key_values,
        ...
        return max(num_frames - 1, 0), past_key_values
