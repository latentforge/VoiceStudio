...
- **Word grouping.** `Dia2Processor._transcribe` groups tokens into words with `transformers.models.whisper.tokenization_whisper._combine_tokens_into_words`, the port of OpenAI Whisper's own `split_to_word_tokens` plus `merge_punctuations`. Upstream `whisper_timestamped` groups them with its own `split_tokens_on_spaces` / `split_tokens_on_unicode`, over a wider punctuation set that also merges `;`, `#`, `$`, `%`, `&`, `*`, `+`, `/`, `<`, `=`, `>`, `@`, `[`, `\`, `]`, `^`, `_`, `` ` ``, `{`, `|`, `}` and `~` into the word before them. The two partition the 82 words of the four measured clips identically, so no divergence has been observed, but a script punctuated with a mark only upstream merges would group differently. The word-final trim upstream applies on top of its grouping is implemented, and is measured in the section above.
