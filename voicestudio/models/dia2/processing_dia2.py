...
class Dia2Processor(ProcessorMixin):
    r"""
    Constructs a Dia2 processor which wraps an [`EncodecFeatureExtractor`], an [`AutoTokenizer`] and a
    [`MimiModel`] into a single processor.
    ...
    Args:
        feature_extractor ([`EncodecFeatureExtractor`]):
            Feature extractor of the Mimi codec, used to resample conditioning audio.
    ...
    def __init__(self, feature_extractor=None, tokenizer=None, audio_tokenizer=None, chat_template=None):
        # `ProcessorMixin` only accepts an `audio_tokenizer` argument that subclasses
        # `PreTrainedAudioTokenizerBase`, which `MimiModel` does not; holding the codec as a plain attribute
        # also keeps its weights out of `save_pretrained`.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        r"""
        Loads the tokenizer from `pretrained_model_name_or_path` and, since Dia2 checkpoints do not bundle the
        codec, the Mimi feature extractor and model from the repository the model config's `audio_tokenizer_id`
        names.

        Args:
            pretrained_model_name_or_path (`str` or `os.PathLike`):
                Repository id or directory of the Dia2 checkpoint.
            kwargs:
                Forwarded to every sub-component's own `from_pretrained`.

        Returns:
            [`Dia2Processor`]: The assembled processor. Its `audio_tokenizer` is `None` when the codec
            repository cannot be reached, which leaves audio conditioning and [`~Dia2Processor.decode`]
            unavailable.
        """
        audio_tokenizer = kwargs.pop("audio_tokenizer", None)
        ...
        if audio_tokenizer_id is not None:
            try:
                feature_extractor = EncodecFeatureExtractor.from_pretrained(audio_tokenizer_id)
                if audio_tokenizer is None:
                    audio_tokenizer = MimiModel.from_pretrained(audio_tokenizer_id)
            except OSError:
                logger.warning_once(
                    f"Could not load the Mimi codec from '{audio_tokenizer_id}'. Audio conditioning and "
                    "`Dia2Processor.decode` stay unavailable until `audio_tokenizer` is set."
                )
    ...
    def _require_audio_tokenizer(self) -> MimiModel:
        if self.audio_tokenizer is None:
            raise ValueError(
                "This Dia2Processor has no `audio_tokenizer`. Set one to a `MimiModel` before encoding "
                "conditioning audio or decoding generated codes."
            )
