...
class Dia2BackboneEmbeddings(nn.Module):
    ...
    contributes on frames where it carries a word token. Every codebook channel owns a slice of one shared audio
    table, selected by a per-codebook offset, and all channels are summed.
    ...
    def __init__(self, config: Dia2Config):
        ...
        rank = config.text_embedding_rank or config.hidden_size
...
class Dia2BackboneModel(LlamaModel, Dia2PreTrainedModel):
    ...
    def forward(self, **super_kwargs) -> BaseModelOutputWithPast:
        ...
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length, 2 + num_codebooks)`):
            ...
            stream, and channels 2 onwards the delayed audio codebooks. Can be obtained from
            [`Dia2Processor`] and [`~generation_dia2.apply_delay_pattern`].
        """
...
class Dia2DepthDecoderModel(Dia2PreTrainedModel):
    ...
    def __init__(self, config: Dia2DepthDecoderConfig):
        ...
        if config.use_text_stream_embedding:
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        text_input_ids (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Main text stream token of this frame. Only used when `config.use_text_stream_embedding` is `True`.
        second_text_input_ids (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Second text stream token of this frame. Only used when `config.use_text_stream_embedding` is `True`.
        ...
        if self.embed_text_tokens is not None and codebook_indices[0] == 0:
            if text_input_ids is None or second_text_input_ids is None:
                raise ValueError(
                    "`text_input_ids` and `second_text_input_ids` are required when "
                    "`config.use_text_stream_embedding` is set and codebook 0 is decoded."
                )
...
class Dia2OutputWithPast(ModelOutput):
    ...
    action_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length, num_actions)`):
    ...
    depth_decoder_logits (`torch.FloatTensor` of shape `(num_frames, num_codebooks - 1, vocab_size)`, *optional*):
        Prediction scores of the remaining codebooks, over the frames the depth decoder was trained on.
    """
...
class Dia2ForConditionalGeneration(Dia2PreTrainedModel, Dia2GenerationMixin):
    ...
    def __init__(self, config: Dia2Config):
        ...
        self.action_head = nn.Linear(config.hidden_size, config.num_actions, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length, num_codebooks)`, *optional*):
            Target codebook tokens, aligned with `input_ids` frame for frame. They are shifted internally, so
            frame `t` is trained to predict frame `t + 1`. Indices set to `-100` are ignored by the loss; a
            frame whose codebooks `1 ..` are all `-100` is skipped by the depth decoder.
        ...
        >>> outputs = model(input_ids=frames, labels=labels)
        ...
        if action_labels is not None:
            action_loss = self.loss_function(
                logits=action_logits, labels=action_labels, vocab_size=self.config.num_actions, **kwargs
            )
            loss = action_loss
    ...
    def _forward_depth_decoder(
        ...
        **kwargs,
        ...
        if self.depth_decoder.config.use_text_stream_embedding and input_ids is not None:
