...
class SparkTTSConfig(Qwen2Config):
    ...
    the task and style control tokens, so this configuration is a [`Qwen2Config`] plus the audio tokenizer that turns
    those tokens into a waveform. It inherits every argument of [`Qwen2Config`]; `vocab_size` covers ordinary text
    tokens as well as the added ones.
    ...
    sub_configs = {"audio_tokenizer_config": SparkTTSBiCodecConfig}
