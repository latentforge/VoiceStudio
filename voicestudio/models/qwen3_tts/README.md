...
The published `Qwen/Qwen3-TTS-*` checkpoints keep the original weight layout, so they need a one-time conversion before they load. `convert` reads a local checkpoint directory:

```python
from huggingface_hub import snapshot_download
from voicestudio.models.qwen3_tts.weight_conversion import convert

convert(snapshot_download("Qwen/Qwen3-TTS-12Hz-1.7B-VoiceDesign"), "qwen3-tts-voicedesign-hf")
```

The processor loads from the original repository, whose `speech_tokenizer` subfolder it converts on first use:

```python
import torch
from voicestudio.models.qwen3_tts import Qwen3TTSForConditionalGeneration, Qwen3TTSProcessor

processor = Qwen3TTSProcessor.from_pretrained("Qwen/Qwen3-TTS-12Hz-1.7B-VoiceDesign")
model = Qwen3TTSForConditionalGeneration.from_pretrained("qwen3-tts-voicedesign-hf", dtype=torch.float16).to("cuda")
